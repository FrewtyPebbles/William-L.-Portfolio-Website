...
vec4 hex_mask_effect(vec4 base_color);

vec4 quantize_color(vec4 base_color, float steps);
...
void main() {
    ...
    base_color.a = min(base_color.a, dot(lighting, luma));

    frag_color = base_color;//quantize_color(base_color, 2.0);
}

vec4 quantize_color(vec4 base_color, float steps) {
    base_color = floor(base_color * steps) / (steps - 1.0);
    return base_color;
}

vec4 hex_mask_effect(vec4 base_color) {
    vec3 clip_ndc = v_clip_pos.xyz / v_clip_pos.w;

    // Remap to UV range (0 to 1)
    vec2 clip_screen_uv = clip_ndc.xy * 0.5 + 0.5;
    float mask_value = float(texture(mask, vec2(clip_screen_uv.x * 3.0 + time/10000.0, clip_screen_uv.y * -3.0 + time/10000.0)).r < 0.5);
